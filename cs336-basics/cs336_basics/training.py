...
def cross_entropy(logits: Float[Tensor, "... batch_size vocab_size"], targets: Int[Tensor, "... batch_size"]) -> Float[Tensor, "..."]:
    ...
    target_logits: Float[Tensor, "... batch_size 1"] = adjusted.gather(-1, targets.unsqueeze(-1).to(torch.int64))
...
def load_checkpoint(source, model: torch.nn.Module, optimizer: torch.optim.Optimizer) -> int:
