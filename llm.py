...
class CausalMultiHeadAttention(torch.nn.Module):
    def __init__(self, d_in: int, d_out: int, n_heads: int, context_length: int, rope_theta: Optional[float] = None):
        ...
        self.variance = (2 / (self.d_in + self.d_out))
        for param in (self.weights_q, self.weights_k, self.weights_v, self.weights_o):
            torch.nn.init.trunc_normal_(param, mean=0, std=math.sqrt(self.variance), a=-3*self.variance, b=3*self.variance)

        mask_buffer = torch.tril(torch.ones(self.context_length, self.context_length)).to(torch.bool)
